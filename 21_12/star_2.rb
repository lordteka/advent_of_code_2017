class Rules
  ...
  def new_square(old_square)
    ...
    old_squares = all_square(old_square)
    rules.each do |rule|
      return rule[OUT] if old_squares.include? rule[IN]
    end
  end
...
p Pattern.new(Rules.new('input.txt')).expand_n_time!(18).shape.inject(0) { |sum, l| sum += l.count('#') }
